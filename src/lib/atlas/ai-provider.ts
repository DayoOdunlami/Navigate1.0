...
async function callOpenAI(
  ...
  const toolCalls = choice.message.tool_calls?.map(tc => {
    try {
      ...
      console.error(`Failed to parse tool call arguments for ${tc.function.name}:`, tc.function.arguments);
      throw new Error(`Failed to parse tool call arguments: ${parseError instanceof Error ? parseError.message : 'Unknown error'}`);
    }
  });
...
async function callAnthropic(
  ...
  // The package is now installed, so this should work
