...
export async function scrapeWithJina(url: string): Promise<ScrapeResult> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Jina Reader failed (${response.status}): ${errorText.substring(0, 200)}`);
    }
    
    const content = await response.text();
    
    // Check if we got an error page instead of content
    if (content.includes('Internal Server Error') || content.includes('Error') && content.length < 500) {
      throw new Error('Jina Reader returned an error page. The URL may be inaccessible or blocked.');
    }
    
    if (!content || content.trim().length === 0) {
      throw new Error('Jina Reader returned empty content. The page may be blocked or require authentication.');
    }
...
// ============================================================================
// CRAWL4AI (Fallback scraper - works in serverless environments)
// ============================================================================

export async function scrapeWithCrawl4AI(url: string): Promise<ScrapeResult> {
  try {
    // Dynamic import - only loads if Crawl4AI is actually used
    const { Crawler } = await import('crawl4ai');
    ...
    const title = titleMatch 
      ? titleMatch[1] 
      : result.metadata?.title || result.metadata?.ogTitle || undefined;
    ...
    console.error('[Atlas] Crawl4AI scraping error:', {
      url,
...
export async function scrapeWithPlaywright(
  ...
  try {
    ...
    throw new Error(`Failed to import Playwright: ${importError instanceof Error ? importError.message : 'Unknown error'}. Make sure Playwright is installed: npm install playwright && npx playwright install chromium`);
  }
  ...
  try {
    ...
    if (response && !response.ok() && response.status() >= 400) {
      await browser.close();
      throw new Error(`Page returned error ${response.status()}: ${response.statusText()}`);
    }
    ...
    // Log error before closing browser
    console.error('[Atlas] Playwright scraping error:', {
      url,
      error: error instanceof Error ? error.message : String(error),
    ...
    try {
      ...
      console.warn('[Atlas] Failed to close browser:', closeError);
    }
  }
}
...
export async function scrape(
  ...
  const playwrightDomains = [
    'gov.uk',  // Has accordions, complex structure
    'www.gov.uk',  // Main gov.uk domain
  ...
  if (method === 'jina') {
    // Check for gov.uk first (most common)
    if (playwrightDomains.some(d => url.includes(d))) {
      selectedMethod = 'playwright';
      console.log(`[Atlas] Auto-selected Playwright for ${url} (gov.uk domain)`);
  ...
  // Single page scrape with fallback logic
  switch (selectedMethod) {
    case 'playwright':
      // For gov.uk domains, try Playwright first, fallback to Crawl4AI
      if (playwrightDomains.some(d => url.includes(d))) {
        try {
          return await scrapeWithPlaywright(url, opts.playwright || config?.playwright);
        } catch (playwrightError) {
          console.warn(`[Atlas] Playwright failed for ${url}, trying Crawl4AI fallback:`, playwrightError);
          try {
            return await scrapeWithCrawl4AI(url);
          } catch (crawl4aiError) {
            throw new Error(
              `All scrapers failed. Playwright: ${playwrightError instanceof Error ? playwrightError.message : 'Unknown'}, ` +
              `Crawl4AI: ${crawl4aiError instanceof Error ? crawl4aiError.message : 'Unknown'}`
            );
          }
        }
      }
